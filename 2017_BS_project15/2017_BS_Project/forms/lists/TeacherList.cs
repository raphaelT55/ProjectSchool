    ...
    public partial class TeacherList : Form
    {
        ...
        private void DataLoad()
        {
            try
            {
                ...
                var select = "SELECT idNUmber,fname,lname,email,password, id FROM users WHERE type = 3 ORDER by id DESC";
        ...
        private void CoursesLoad()
        {
            ...
            try
            {
                ...
                con.Open();
                    ...
                    string query = "SELECT id,name FROM course WHERE lecture = 1";
        ...
        public void addTeacher(string fname, string lname, string email, string p, string id, int type)
        {
            DBA db = DBA.Instance;
            db.connect();
            db.query("INSERT INTO users (fname, lname, email, password, idNUmber, type) VALUES ('" + fname + "', '" + lname + "', '" + email + "', '" + p + "', '" + id + "', 3)");
        }
        ...
        private void add_new_btn_Click(object sender, EventArgs e)
        {
            ...
            {
                addTeacher(fname.Text, lname.Text, email.Text, password.Text, IdNumber.Text, 3);
                DataLoad();
            }
            else
        ...
        public void deleteTeacher(string ID)
        {
            DBA db = DBA.Instance;
            db.connect();
            db.query("DELETE  users where IdNUmber = " + ID + " and type = 3");
        }
